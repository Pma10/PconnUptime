...
class Lookup(Cog):
    def __init__(self, app):
        ...
        self.today_most_player = [0, datetime.now()]
        self.today_most_ping = [0, datetime.now()]
    ...
    async def check_server_status(self):
        try:
            ...
            if player > self.today_most_player[0]:
                self.today_most_player = [player, str(datetime.now())]
                await MongoDB.update_max('max_players', today_date, player)

            if ping > self.today_most_ping[0]:
                self.today_most_ping = [ping, str(datetime.now())]
                await MongoDB.update_max('max_ping', today_date, ping)
            ...
            embed.description = f'\n **{status.motd.parsed[14]}** \n \n **핑** \n > {ping}ms \n **최대 핑** \n > {self.today_most_ping[0]}ms \n **버전** \n > {status.version.name}\n **접속자**\n > {player}/{max_players}명 \n **최고 동접** \n > {self.today_most_player[0]}/{max_players}명'
